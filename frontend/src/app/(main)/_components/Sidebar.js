...
export default function Sidebar(){
	...
	return (
        <>
		<div className={`${isSideBarOpen ? 'block':'hidden'} fixed inset-0 z-20 transition-opacity bg-black opacity-50 lg:hidden`} onClick={toggleSidebar} ></div>
    
        <div className={`${isSideBarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'} fixed inset-y-0 left-0 z-30 w-64 overflow-y-auto transition duration-300 transform bg-gray-900 lg:translate-x-0 lg:static lg:inset-0`} >
